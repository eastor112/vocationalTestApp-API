...
const updateUser = async (id, rest) => {
  ...
  if (rest.profile && user.profile !== '') {
    const lastProfileArr = user.profile.split('/');
    const lastprofile = lastProfileArr[lastProfileArr.length - 1];
    const cloudinaryId = lastprofile.split('.')[0];
